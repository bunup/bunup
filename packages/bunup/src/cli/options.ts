...
const program = cli()
	.name("bunup")
	.version(version)
	.description("A blazing-fast build tool for your TypeScript/React libraries - built on Bun")
	.with({
		ignoreOptionDefaultValue: true,
	})
	.example([
		pc.gray(`${pc.blue("bunup")}                            # Basic build`),
		pc.gray(`${pc.blue("bunup src/index.ts")}               # Single entry file`),
		pc.gray(`${pc.blue("bunup src/**/*.ts")}                # Glob pattern for multiple files`),
		pc.gray(`${pc.blue("bunup --watch")}                    # Watch mode`),
		pc.gray(`${pc.blue("bunup --format cjs,esm")}           # Multiple formats`),
		pc.gray(`${pc.blue("bunup --target bun")}               # Bun target`),
		pc.gray(`${pc.blue("bunup src/cli.ts")}                 # Multiple entries`),
		pc.gray(`${pc.blue("bunup --dts.splitting")}            # Declaration splitting`),
		pc.gray(`${pc.blue("bunup --dts-only")}                 # Only emit declaration files`),
		pc.gray(
	...
	.option(
		...
		z
			.union(
				...
				z.boolean().describe("Whether to use a configuration file").default(true),
			)
			.optional(),
	)
	.option(
	...
	.option(
		...
		z
			.string()
			.describe("Name of the build configuration (for logging and identification)")
			.example("my-library")
			.optional(),
	)
	.option(
		"out-dir",
		z.string().describe("Output directory for bundled files").alias("o").default("dist"),
	)
	.option(
		"format",
		z
			.union(
				z.string().choices(["esm", "cjs", "iife"]).describe("Single output format"),
				z.array(z.string().choices(["esm", "cjs", "iife"])).describe("Multiple output formats"),
			)
			.alias("f")
			.default("esm"),
	)
	...
	.option(
		...
		z
			.boolean()
			.describe("Enable all minification options (whitespace, identifiers, syntax)")
			.optional(),
	)
	.option(
		"minify-whitespace",
		z.boolean().describe("Minify whitespace in the output to reduce file size").optional(),
	)
	.option(
		"minify-identifiers",
		z.boolean().describe("Minify identifiers by renaming variables to shorter names").optional(),
	)
	.option(
		"minify-syntax",
		z.boolean().describe("Minify syntax by optimizing code structure").optional(),
	)

	.option(
		"watch",
		z.boolean().describe("Watch for file changes and rebuild automatically").optional(),
	)
	.option("clean", z.boolean().describe("Clean the output directory before building").default(true))
	.option(
		"silent",
		z.boolean().describe("Disable logging during the build process").alias("q").optional(),
	)
	...
	.option(
		"conditions",
		z.array(z.string()).describe("Package.json export conditions for import resolution").optional(),
	)
	.option(
		"target",
		z
	...
	.option(
		"external",
		z.array(z.string()).describe("External packages that should not be bundled").optional(),
	)
	.option(
		"no-external",
		z
		...
		z
			.union(
				...
				z.object({
					entry: z
						.union(
							z.string().describe("Single entrypoint for declaration file generation"),
							z.array(z.string()).describe("Multiple entrypoints for declaration file generation"),
						)
						.optional(),
					resolve: z
						.union(
							...
							z
								.array(z.string())
								.describe("Names or patterns of packages from which to resolve types"),
						)
						.optional(),
	...
	.option(
		...
		z
			.boolean()
			.describe("Only emit TypeScript declaration files without building JavaScript output")
			.optional(),
	)
	.option(
		"preferred-tsconfig",
	...
	.option(
		...
		z
			.union(
				z.boolean().describe("Generate a sourcemap (uses the inline type by default)"),
				z
					.string()
		...
		z
			.union(
				...
				z
					...
					.example('--env.NODE_ENV="production" --env.API_URL="https://api.example.com"'),
			)
			.optional(),
	)
	.option("banner", z.string().describe("Banner text added to the top of bundle files").optional())
	.option(
		"footer",
		z.string().describe("Footer text added to the bottom of bundle files").optional(),
	)
	.option(
		"drop",
		z
	...
	.option(
		...
		z.boolean().describe("Ignore dead code elimination annotations (@__PURE__, sideEffects)"),
	)
	.option(
		"emit-dce-annotations",
		z.boolean().describe("Force emit @__PURE__ annotations even with minification"),
	)

	.option("on-success", z.string().describe("Command to run after successful build").optional())
		...
		z
			.union(
				...
				z.object({
					...
					"exclude-cli": z
						.boolean()
						.describe("Whether to exclude CLI entry points (cli/bin files) from exports field")
						.default(true),
					"exclude-css": z
						.boolean()
			...
			.union(
				...
				z.object({
					level: z
						.string()
						.choices(["warn", "error"])
						.describe("The level of reporting for unused or incorrectly categorized dependencies")
						.default("warn"),
...
export const parseCliOptions = (argv: string[]): CliOnlyOptions & Partial<BuildOptions> => {
