...
export const handleError = (error: unknown, context?: string): void => {
	...
	const knownError = KNOWN_ERRORS.find(
		(error) =>
			error.pattern.test(errorMessage) && (error.errorType === errorType || !error.errorType),
	);
...
export const formatNoEntryPointsFoundError = (defaultEntrypoints: string[]): string => {
	return (
...
export const formatInvalidEntryPointsError = (userEntrypoints: string[]): string => {
...
export function formatBunBuildError(error: BuildMessage | ResolveMessage): string {
