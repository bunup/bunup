...
import type { BunupPlugin, OnBuildDoneCtx, OnBuildStartCtx } from "../../src/plugins/types";
...
describe("Bunup Plugin", () => {
	...
	it("should execute hooks from multiple plugins in the correct order", async () => {
		...
		expect(result.success).toBe(true);
		expect(callOrder).toEqual(["first-start", "second-start", "first-done", "second-done"]);
	});
	...
	it("should handle async plugin hooks correctly", async () => {
		...
		expect(result.success).toBe(true);
		expect(callOrder).toEqual(["start", "start-after-delay", "done", "done-after-delay"]);
	});
	...
	it("should handle plugins with empty hooks object", async () => {
		...
		expect(result.success).toBe(true);
		expect(validateBuildFiles(result, { expectedFiles: ["index.mjs"] })).toBe(true);
	});
	...
	it("should correctly set entrypoint for entry-point files in output", async () => {
		...
		const entryPointFile = buildContext.files.find(
			(file) => file.kind === "entry-point" && file.fullPath.endsWith("index.mjs"),
		);
	...
	it("should correctly handle multiple entrypoints in output files", async () => {
		...
		const entryPointFiles = buildContext.files.filter((file) => file.kind === "entry-point");

		expect(entryPointFiles.length).toBe(2);

		const indexFile = entryPointFiles.find((file) => file.fullPath.endsWith("index.mjs"));
		const otherFile = entryPointFiles.find((file) => file.fullPath.endsWith("other.mjs"));
	...
	it("should generate chunk files with undefined entrypoints when code splitting is enabled", async () => {
		createProject({
			"src/shared.ts": "export const shared: { value: number } = { value: 42 };",
		...
		const indexFile = entryPointFiles.find((file) => file.fullPath.includes("index.mjs"));
		const otherFile = entryPointFiles.find((file) => file.fullPath.includes("other.mjs"));
		...
		const chunkFiles = buildContext.files.filter((file) => file.kind === "chunk");
	...
	it("should generate dts chunk files with undefined entrypoints when dts splitting is enabled", async () => {
		...
		const dtsEntryPoints = dtsFiles.filter((file) => file.kind === "entry-point");
		expect(dtsEntryPoints.length).toBe(2);

		const indexDtsFile = dtsEntryPoints.find((file) => file.fullPath.includes("index.d.mts"));
		const otherDtsFile = dtsEntryPoints.find((file) => file.fullPath.includes("other.d.mts"));
	...
	it("should correctly map entrypoints in complex projects with multiple shared modules", async () => {
		...
		const chunkFiles = buildContext.files.filter((file) => file.kind === "chunk" && !file.dts);
		const dtsEntryPointFiles = buildContext.files.filter(
			(file) => file.kind === "entry-point" && file.dts,
		);
		const dtsChunkFiles = buildContext.files.filter((file) => file.kind === "chunk" && file.dts);
		...
		const mainFile = entryPointFiles.find((file) => file.fullPath.includes("main.mjs"));
		const adminFile = entryPointFiles.find((file) => file.fullPath.includes("admin.mjs"));
		const clientFile = entryPointFiles.find((file) => file.fullPath.includes("client.mjs"));
		...
		expect(chunkFiles.every((file) => file.entrypoint === undefined)).toBe(true);
		expect(chunkFiles.every((file) => file.pathRelativeToOutdir.startsWith("shared/"))).toBe(true);
		expect(dtsChunkFiles.every((file) => file.entrypoint === undefined)).toBe(true);
		expect(dtsChunkFiles.every((file) => file.pathRelativeToOutdir.startsWith("shared/"))).toBe(
			true,
		);

		const mainDtsFile = dtsEntryPointFiles.find((file) => file.fullPath.includes("main.d.mts"));
		const adminDtsFile = dtsEntryPointFiles.find((file) => file.fullPath.includes("admin.d.mts"));
		const clientDtsFile = dtsEntryPointFiles.find((file) => file.fullPath.includes("client.d.mts"));
		...
		expect(
			allFiles.every((file) =>
				file.dts
					? file.kind === "chunk"
						? file.pathRelativeToOutdir.startsWith("shared/") && file.fullPath.includes(".d.ts")
						: file.fullPath.includes(".d.mts")
					: file.kind === "chunk"
						? file.pathRelativeToOutdir.startsWith("shared/") && file.fullPath.includes("chunk-")
