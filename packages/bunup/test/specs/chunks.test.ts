...
describe("chunk splitting", () => {
	it("should generate correctly named chunks and assets in the specified output directory", async () => {
		...
		expect(result.files.some((file) => file.path.includes("shared/chunk-"))).toBe(true);
		expect(result.files.some((file) => file.path.includes("plain-"))).toBe(true);
		expect(result.files.some((file) => file.path.includes("/index.mjs"))).toBe(true);
	});
	...
	it("chunk imports should point to valid path", async () => {
		...
		const dtsFile = result.files.find((file) => file.extension === ".d.ts" && file.name === "a");
	...
	it("should place JS and DTS chunk files in the shared folder, and not place assets in shared", async () => {
		...
		const jsChunkFiles = result.files.filter(
			(f) =>
				f.path.includes("shared/chunk-") && (f.path.endsWith(".js") || f.path.endsWith(".mjs")),
		);
		...
		const dtsChunkFiles = result.files.filter(
			(f) =>
				...
				(f.path.endsWith(".d.ts") || f.path.endsWith(".d.mts") || f.path.endsWith(".d.cts")),
		);
	...
	it("should only create one dts chunk file per chunk regardless of formats", async () => {
		...
		for (const base of chunkBaseNames) {
			const filesForChunk = dtsChunkFiles.filter((f) => f.path.includes(`shared/${base}.d.`));
